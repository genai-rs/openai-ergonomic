...
# Check recent commits for Claude/AI attribution
...
if grep -r --include="*.rs" --include="*.toml" --include="*.md" -iE "(generated by claude|created by claude|ai-generated|llm-generated)" . 2>/dev/null | grep -v "check-attribution.sh" | grep -v "AGENTS.md" | grep -v "reviewer_driver.md"; then
...
if [ $ATTRIBUTION_FOUND -eq 0 ]; then
    ...
    echo "Please remove all references to Claude, AI generation, or LLM assistance."
